...
> **注意**：記得把 Google OAuth app 設為 **Production（正式版）**，否則授權每 7 天會失效。
> **Note**: set your Google OAuth app to **Production**, otherwise the authorization expires every 7 days.
...
**注意**：附帶的期刊資料 `data/scimago.csv.gz` 來自 **SCImago（源自 Elsevier Scopus）**，僅供**非商業**學術／個人用途，**不在 MIT 授權範圍內**；若要再利用請標示 SCImago 出處並確認其條款。
